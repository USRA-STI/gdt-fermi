...
The localization fit procedure is performed in the :meth:`~gdt.missions.fermi.gbm.localization.dol.legacy_functions.find_best_location`
method using a :math:`\chi^2` minimization routine. The :math:`\chi^2` is defined as

.. math::

    \chi^2 = \Sigma_i \frac{(M_i - E_i)^2}{E_i}

where :math:`M_i` is the measured counts and :math:`E_i`
...
:math:`B_i` are typically determined from a polynomial fit to background periods (:ref:`See Example<dol-example-background>`).
:math:`S_i` consists of the detector response :math:`R_i` multiplied by the flux :math:`f` of the source
